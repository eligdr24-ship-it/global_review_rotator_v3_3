...
## v3.18.2 Admin Visual Match Update

- Admin dashboard visual style updated to more closely match the approved mockup.
- Added dark navy sidebar with logo, app name, navigation icons, and admin profile card.
- Dashboard cards were restyled with white surfaces, rounded corners, soft shadows, tighter spacing, and clearer icon/title treatment.
- Completed History remains open inside the dashboard.
- User Management remains a compact card that opens a modal window.
- Performance Overview uses the existing multi-operator chart and receives improved card/chart styling.
- Mobile admin layout stacks compactly with reduced spacing and smaller cards.
- Worker dashboards, Operator Analytics, backend logic, export logic, upload flow, history, trash, and user-management behavior were not intentionally changed.
