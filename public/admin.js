...
function renderUsers(users){const sorted=(users||[]).slice().sort((a,b)=>(b.totalDone||0)-(a.totalDone||0)||String(a.name||'').localeCompare(String(b.name||'')));$('userCards').innerHTML=sorted.map((u,i)=>`<div class="user-card leaderboard-compact-row"><div class="user-avatar" style="background:${esc(u.color)}">${i+1}</div><div><b>${esc(u.name)}</b><p>${u.totalDone||0} selected range • ${u.doneToday||0} today</p></div><strong class="leaderboard-count">${u.totalDone||0}</strong></div>`).join('')||'<p class="small">No operators yet.</p>'}
