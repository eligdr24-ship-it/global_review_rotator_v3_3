...
function smoothPath(points){
  if(points.length<2) return points.map((p,i)=>`${i?'L':'M'}${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(' ');
...
function drawChart(daily){
  const svg=$('dailyChart'),rows=(daily||[]).slice();
  if(!svg) return;
  const W=700,H=280,P=34;
  if(!rows.length){svg.innerHTML='<text x="24" y="48" fill="currentColor">No data yet</text>';return}
  const max=Math.max(1,...rows.map(d=>d.doneCount));
  const pts=rows.map((d,i)=>{const x=P+(rows.length===1?0:i*(W-P*2)/(rows.length-1));const y=H-P-(d.doneCount/max)*(H-P*2);return{x,y,d}});
  const line=smoothPath(pts);
  const area=`${line} L${pts[pts.length-1].x.toFixed(1)},${H-P} L${pts[0].x.toFixed(1)},${H-P} Z`;
  const grid=[0,1,2,3].map(i=>{const y=P+i*(H-P*2)/3;return `<line class="chart-grid" x1="${P}" x2="${W-P}" y1="${y.toFixed(1)}" y2="${y.toFixed(1)}"/>`}).join('');
  const dots=pts.map((p,i)=>`<circle class="chart-dot" cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="${i===pts.length-1?5:3.5}"><title>${esc(p.d.day)}: ${p.d.doneCount}</title></circle>`).join('');
  const labels=pts.filter((_,i)=>i===0||i===pts.length-1||i%Math.ceil(pts.length/5)===0).map(p=>`<text class="chart-label" x="${p.x.toFixed(1)}" y="${H-8}" text-anchor="middle">${esc(String(p.d.day||'').slice(5))}</text>`).join('');
  svg.innerHTML=`<defs><linearGradient id="gradLine" x1="0" x2="1"><stop offset="0" stop-color="#7c3aed"/><stop offset=".55" stop-color="#007aff"/><stop offset="1" stop-color="#22c55e"/></linearGradient><linearGradient id="gradArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#7c3aed" stop-opacity=".26"/><stop offset=".55" stop-color="#007aff" stop-opacity=".10"/><stop offset="1" stop-color="#7c3aed" stop-opacity="0"/></linearGradient></defs>${grid}<path class="chart-area" d="${area}"/><path class="chart-line" d="${line}"/>${dots}${labels}`;
}
