...
async function loadReport(){const j=await api(`/api/report?${dateParams()}&user=${USER_ID}`);LAST=j;fill(j.status);renderWeeklyGoalBox(j.weekly);renderHistory(j.report.history);drawChart(j.report.daily);renderActivity(j.report.history)}
