...
function renderHistory(hist){$('historyRows').innerHTML=hist.length?hist.map(h=>`<tr data-history-id="${esc(h.id||'')}"><td>${esc(localTime(h.at))}</td><td>${esc(h.day)}</td><td>${esc(h.source||'Links')}</td><td class="break"><a href="${esc(h.link)}" target="_blank">${esc(h.link)}</a></td><td class="break"><input class="input history-edit submitted-edit" data-history-id="${esc(h.id||'')}" value="${esc(h.submittedLink||'')}" placeholder="Paste submitted link"></td><td>${esc(h.text)}</td></tr>`).join(''):'<tr><td colspan="6">No submissions for this date range.</td></tr>';setText('resultCount',`Showing ${hist.length} result${hist.length===1?'':'s'}`);bindHistoryInputs()}
function bindHistoryInputs(){document.querySelectorAll('.history-edit').forEach(el=>{el.oninput=()=>{el.classList.add('dirty');const btn=$('saveHistoryBtn');if(btn)btn.textContent='Save All Changes *';}})}
async function saveHistoryChanges(){const updates=[];document.querySelectorAll('.submitted-edit').forEach(el=>{const id=el.dataset.historyId;if(id)updates.push({id,submittedLink:el.value});});if(!updates.length)return alert('No rows to save.');const btn=$('saveHistoryBtn');try{if(btn){btn.disabled=true;btn.textContent='Saving...'}const r=await api('/api/history/update',{method:'POST',body:JSON.stringify({userId:USER_ID,updates})});document.querySelectorAll('.history-edit').forEach(el=>{el.classList.remove('dirty');el.classList.add('saved');setTimeout(()=>el.classList.remove('saved'),900)});if(LAST?.report?.history){for(const u of updates){const row=LAST.report.history.find(h=>String(h.id)===String(u.id));if(row)row.submittedLink=u.submittedLink;}}if(btn)btn.textContent='Saved ✅';setTimeout(()=>{if(btn)btn.textContent='Save All Changes'},1000);}catch(e){alert(e.message)}finally{if(btn)btn.disabled=false}}
